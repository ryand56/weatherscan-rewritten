...
const Display = ({
  ...
  const [currentInfo, setCurrentInfo] =
    React.useState<Partial<CurrentCond>>(currentDefaults);
  ...
  React.useEffect(() => {
    if (isReady) {
      if (location !== "") {
        getMainLocation(location, language)
          .then((data) => {
            setLocInfo(data);
          })
          .catch((err) => {
  ...
  const fetchCurrent = (lat: number, lon: number) => {
    getCurrentCond(lat, lon, language)
      .then((data) => {
        setCurrentInfo(data);
      })
      .catch((err) => {
  ...
  return (
    <div
      id="main"
      ref={mainRef}
      className="relative top-1/2 left-1/2 overflow-hidden w-main h-main"
    >
      <img
      ...
      <LogoArea />
      <InfoMarquee
        top="Some place: 72° & sunny | Some other place: 68° & cloudy | Yet another place: 70° & partly cloudy"
        bottom="This project is open source. | Check it out at github.com/elementemerald/weatherscan-rewritten |"
      />
      {focusedAlert && (
        <MarqueeSevere top={focusedAlert.status} bottom={focusedAlertText} />
      )}
    </div>
  );
};
